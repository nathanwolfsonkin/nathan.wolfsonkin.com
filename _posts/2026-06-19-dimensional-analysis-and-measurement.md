...
Imagine that we used force as defined in newtons as a primary dimension. We know from empirical measurements and Euler's first law that:
$$F=ma$$
...
It might be possible that the tall locker has a greater total volume than the shorter ones, but it likely doesn't store items as effectively due to its limited width. To develop a suitable metric for locker quality, we need to define some properties. To simplify the problem, let us assume a depth that is reasonable for item storage, $d \gg 0$. We want quality to decrease when height or width approach zero, since that limits a locker's ability to effectively store items. More formally we can write:
...
If we had an infinitely large locker, we could store infinite items. Therefore locker quality should approach infinity as the width or height approach infinity. More formally:
...
| Angular amplitude of swing | $\theta$ | No dimensions |
...
[1] P. Xin, Z. Xie, and P. Dames, “Towards Predicting Collective Performance in Multi-Robot Teams,” IEEE Trans. Robot., vol. 41, pp. 5229–5245, 2025, doi: 10.1109/TRO.2025.3600164.
